...
async function main() {
  assert.equal(fs.realpathSync(process.execPath).toLowerCase(), fs.realpathSync(path.join(runtimeRoot, process.platform === 'win32' ? 'node.exe' : 'node')).toLowerCase(), 'Use the bundled runtime Node executable');
