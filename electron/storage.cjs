...
const { validateBaseUrl, validateHarnessUrl, validateModel, validateThinking } = require('./chat-client.cjs');

const DEFAULT_SETTINGS = Object.freeze({ model: 'deepseek-flash', thinking: false, baseUrl: 'https://api.deepseek.com', theme: 'system', harnessUrl: 'http://127.0.0.1:3080', workspace: null });
...
class Storage {
  ...
  getApiKey() {
    if (!this.settings.encryptedApiKey) throw new Error('请先在设置中添加 DeepSeek API Key。');
