...
async function setMode(next){mode=next;$('chat-mode').setAttribute('aria-selected',String(next==='chat'));$('code-mode').setAttribute('aria-selected',String(next==='codex'));$('chat-view').classList.toggle('hidden',next!=='chat');$('code-view').classList.toggle('hidden',next!=='codex');$('header-title').textContent='DeepSeek';$('header-subtitle').textContent=next==='chat'?'Chat':'Harness';$('new-label').textContent=next==='chat'?'新建对话':'Chat 新对话';if(desktop)await desktop.setMode(next);updateBounds();}
...
function newChat(){currentId=null;attachment=null;$('prompt').value='';renderAttachment();renderHistory();renderMessages();setMode('chat');$('prompt').focus();}
function selectSession(id){currentId=id;attachment=null;$('prompt').value='';renderAttachment();renderHistory();renderMessages(true);setMode('chat');}
...
document.addEventListener('keydown',e=>{if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='k'){e.preventDefault();if(!document.querySelector('dialog[open]'))actions.search();}if((e.ctrlKey||e.metaKey)&&e.key.toLowerCase()==='n'){e.preventDefault();if(!document.querySelector('dialog[open]'))newChat();}});
